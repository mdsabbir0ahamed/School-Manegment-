...
20. **Payment Receipt PDF Generator** — `GET /api/finance/transactions/:id/receipt` (`requireFinance`): generates an A5 PDF receipt using PDFKit. Layout: indigo header banner with school name + "PAYMENT RECEIPT" subtitle, receipt number + timestamp meta row, student name section, payment details grid (invoice no., fee type, period, date, method, reference ID, notes), green amount box with large BDT amount and a "PAID" circle stamp, computer-generated footer. Uses `BDT` prefix (Helvetica-safe, no Bengali glyphs). Finance Transactions tab: new "Receipt" 7th column with a `FileText` icon + "Receipt" link per row; spinner shown while downloading; blob downloaded and auto-triggered via `<a>` click; toast on success/failure.
