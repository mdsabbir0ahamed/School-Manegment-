...
31. **Student Portal (`/student`, STUDENT role only)** — Dedicated self-service portal for logged-in students. DB: `linked_student_id` (integer, nullable) added to `usersTable`; demo student1@school.edu linked to student id=1. Four backend endpoints (`requireAuth` + `role === STUDENT`): `GET /api/student/me` (profile + class name/section/gradeLevel + all-time attendance stats + exam summary), `GET /api/student/attendance` (last 90 days records + per-status counts + attendance rate), `GET /api/student/results` (all exam results joined with subjects + average % + best subject), `GET /api/student/timetable` (class timetable joined with subjects + teachers, ordered by day+time). Frontend `StudentPortalPage.tsx`: gradient header banner (name, studentId, class, grade, status badge); 4 KPI cards (Attendance Rate, Classes Attended, Exams Taken, Average Score); 4 tabs — Overview (profile dl + recent attendance + recent results + 90-day breakdown progress bars), Attendance (full scrollable table with colour-coded status pills + check-in time + method), Results (full table with subject, exam type pills, marks, %, letter grade, remarks), Timetable (grouped by day with colour-coded day headers, time + subject + teacher + room). "No linked student" empty-state when `linked_student_id` is null. Role guard: STUDENT role's `allowedRoutes` expanded to include `/student` and `/settings`; STUDENT users see "My Portal" nav item in the sidebar; non-STUDENT callers receive 403.
30. **Fee Statement Scheduler (Finance → Statement Activity tab → Scheduler Settings card)** — Cron-based monthly bulk-email of branded PDF fee statements to all active students' parents. DB table: `fee_statement_schedule` (`id`, `tenantId`, `isEnabled`, `dayOfMonth` 1-28, `hour` 0-23, `lastRunAt`, `lastRunCount`, `lastRunErrors`, `updatedAt`). Cron (`lib/statement-scheduler-cron.ts`): runs hourly via `setInterval`; checks day-of-month + hour match before firing; skips if already ran today; resolves parent email via linked parent user → `student.parentEmail` → skips if neither; generates full branded A4 PDF (same quality as manual statement) into a Buffer; sends HTML email with PDF attached using DB SMTP → env-var fallback; logs each dispatch to `fee_statement_logs`; updates `lastRunAt`/`lastRunCount`/`lastRunErrors`. Three endpoints (`requireSuperAdmin`): `GET /api/finance/statement-schedule` (current config), `PUT /api/finance/statement-schedule` (update isEnabled/dayOfMonth/hour with validation), `POST /api/finance/statement-schedule/trigger` (force immediate run, returns `{sent, errors, skippedNoEmail}`). Frontend: collapsible "Monthly Statement Scheduler" card at top of Statement Activity tab — collapsed by default showing status badge and summary; expanded shows enable/disable Switch, Day of Month select (1-28 with ordinal labels), Hour select (0-23 with AM/PM labels), last-run info strip, green run-result banner, "Save Settings" + "Run Now" buttons (with spinners), and a "How it works" violet explainer. Card is hidden automatically for non-SUPER_ADMIN (API returns 403 which sets `schedErr=true`). `startStatementSchedulerCron()` wired into `artifacts/api-server/src/index.ts`.
29. **Statement Activity Report (Finance → Statement Activity tab)** — School-wide statement dispatch ledger. Backend: `GET /api/finance/statement-activity` (`requireFinance`) with query params `dateFrom`, `dateTo`, `action` (PDF_DOWNLOAD/EMAIL_SENT), `triggeredByUserId`, `page`, `limit`. Joins `feeStatementLogsTable` with `studentsTable` and `usersTable`; runs a parallel KPI aggregate (`COUNT GROUP BY action`) and distinct-student count (always unfiltered for global totals); returns `{ logs[], total, kpis{totalDownloads, totalEmails, uniqueStudents, totalDispatches}, staffList[] }`. Frontend: `StatementActivityTab` component with 4 KPI cards (Total Dispatches/PDFs Downloaded/Emails Sent/Students Reached), filter row (date-from, date-to, action Select, staff Select populated from `staffList`), "Clear" + "Refresh" buttons, paginated table (Timestamp/Student/Action badge/Triggered By/Sent To/Delivery Mode), full pagination controls. Action column uses colour-coded pill badges (indigo=PDF, green=Email); Delivery column shows Delivered vs Log-only badge. Empty state and loading skeletons. `statementActivityRouter` registered in `routes/index.ts`.
