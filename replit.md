...
17. **Dashboard Escalation Banner** — `GET /api/dashboard/escalation-summary` (SUPER_ADMIN + ACCOUNTANT only) returns criticalCount, warningCount, totalEscalated, totalAtRisk split by level. Full-width banner card on the Dashboard between the stats grid and charts row, visible to finance roles: amber/red border when alerts exist, green "All Clear" when none; CRITICAL count (red pill), WARNING count (amber pill), ৳ at-risk, "View Escalations →" link to /finance; auto-refreshes every 60 s.
