...
28. **Admin Fee Statement PDF + Email to Parent** — Extends the admin student view (Students page → View dialog) with two quick-action buttons in the dialog header (SUPER_ADMIN/ACCOUNTANT only): "Statement PDF" downloads the existing `GET /parent/fee-statement/:studentId/pdf` directly from the header without switching tabs; "Email to Parent" calls a new `POST /parent/fee-statement/:studentId/email` endpoint (requireFinance guard) that resolves the recipient via linked parent user account → student.parentEmail → 422 if neither found, generates the full A4 PDF into a Buffer (invoice history + payment history tables), sends an HTML email with the PDF attached using the same SMTP priority chain (DB config → env vars → log-only), returns `{ sent, sentTo, deliveryMode }`. The "Email to Parent" button is disabled (with tooltip) when no parent email is on file. The "Statement PDF" button also added to the `FeeLedgerTab` toolbar (inside the Fee Ledger tab) alongside the year filter. Both buttons show spinner while in-flight. `Download` icon added to lucide-react imports. `usersTable` added to `fee-statement.ts` imports.
